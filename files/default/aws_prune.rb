...
# Used to decommission a Sensu client if the node does not exist (EC2)
class Ec2Node < Sensu::Handler
  def filter; end

  def handle
    if ec2_node_exists?(@event['client']['instance_id'])
  ...
  def ec2_node_exists?(instance_id)
    ...
    instance_ids = running_instances.reject { |s| instance_id != s['id'] }.collect \
      { |s| Hash['id', s['id'], 'tags', s['tags']] }
