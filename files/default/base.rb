...
# Base handler provides several methods to be used for creating handlers as well as providing
# some logic to handle when events should alert.
class BaseHandler < Sensu::Handler
  ...
  def description(maxlen = 0)
    ...
    description ||= \
      [@event['client']['name'], @event['check']['name'], @event['check']['output']].join(' : ')
    if event_is_critical? || event_is_warning?
      ...
      if maxlen > 0 && (toadd.length + description.length) > maxlen
        ...
        description = description[0..description_size] if description_size > 0
      end
...
BODY
  ...
  def timeout_and_retry(&block)
    2.times do
      ...
      rescue Timeout::Error.new(msg = 'Timer expired')
        puts msg
      end
      do_sleep
    end
